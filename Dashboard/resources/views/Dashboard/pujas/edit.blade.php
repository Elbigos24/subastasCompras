...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="fw-bold m-0">Editar Puja</h2>
...
<div class="table-card p-4">
    <form action="{{ route('pujas.update', $puja) }}" method="POST">
        @csrf @method('PUT')
        ...
        <div class="row g-3">
                ...
                <select name="user_id" class="form-select @error('user_id') is-invalid @enderror">
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->id }}"
                            {{ old('user_id', $puja->user_id) == $user->id ? 'selected' : '' }}>
                            {{ $user->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <select name="subasta_id" class="form-select @error('subasta_id') is-invalid @enderror">
                    ...
                    @foreach($subastas as $subasta)
                        <option value="{{ $subasta->id }}"
                            {{ old('subasta_id', $puja->subasta_id) == $subasta->id ? 'selected' : '' }}>
            ...
            <div class="col-md-4">
                ...
                <input type="number" name="amount" value="{{ old('amount', $puja->amount) }}" min="1"
            ...
            <div class="col-md-4">
                ...
                <div class="input-group">
                    ...
                    <input type="number" name="price" value="{{ old('price', $puja->price) }}" min="1"
            ...
            <div class="col-md-4">
                ...
                <input type="datetime-local" name="date_hour"
                    value="{{ old('date_hour', \Carbon\Carbon::parse($puja->date_hour)->format('Y-m-d\TH:i')) }}"
        ...
        <div class="mt-4 d-flex gap-2">
            <button type="submit" class="btn btn-warning text-white">
                <i class="fas fa-save me-1"></i> Actualizar Puja
            </button>
